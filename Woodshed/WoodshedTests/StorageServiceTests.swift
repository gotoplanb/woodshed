...
struct StorageServiceTests {
    ...
    @Test func saveSetlistWithSongs() {
        let (storage, tempDir) = makeTempStorage()
        defer { cleanup(tempDir) }

        var setlist = Setlist(title: "Appetite")
        setlist.songs = [
            SongEntry(title: "Welcome to the Jungle", appleMusicID: "123", instrument: "Guitar", sections: [
                Section(title: "Intro", startTime: 0, endTime: 15),
                Section(title: "Verse 1", startTime: 15, endTime: 45, role: "Rhythm"),
            ]),
        ]
        storage.save(setlist)
        ...
        #expect(fresh.setlists[0].songs.count == 1)
        #expect(fresh.setlists[0].songs[0].sections.count == 2)
        #expect(fresh.setlists[0].songs[0].sections[1].role == "Rhythm")
    }
